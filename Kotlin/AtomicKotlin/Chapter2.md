...
코드를 작성하기 전에 테스트를 먼저 작성해 실패시킨 후, 나중에 테스트를 통과하도록 코드를 작성하는 기법이다.
...
프로퍼티 값을 얻기 위해 사용하는 접근자 : getter()
프로퍼티 값을 갱신하기 위한 접근자: setter()
...
세터를 private으로 하고 게터를 public으로 하는 것 가능.
이러면 클래스 밖에서 프로퍼티 값을 읽을 수는 있지만, 변경은 클래스 내부에서만 가능함.
